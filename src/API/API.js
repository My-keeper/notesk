...
class API {
    ...
    static singUp (firstName, lastName, userName, email, password, country, province, city, ZipCode, onSuccess, onFail) {
        ...
        axios.post(`http://18.116.242.41:9000/Users`, {
            firstName : firstName,
            lastName ,
            userName,
            email ,
    ...
    static isLoggedIn(onSuccess) {
        return axios.get('http://18.116.242.41:9000/Users/me', {
    ...
    static login(email, password, onSuccess, onFail) {
        return axios.post('http://18.116.242.41:9000/Users/login', {
            email,
            password
            }).then((res) => {
                localStorage.setItem("token", res.data.token);
    ...
    static Logout() {  
        return axios.post('http://18.116.242.41:9000/Users/logout',{
            headers: {Authorization: localStorage.getItem("token") },
        })
    }
    ...
    static CreateNote(title, content,onSuccess) {
        return axios
            .post('http://18.116.242.41:9000/Notes', {
                title,
                content
            }, {
                headers: {Authorization: localStorage.getItem("token") },
    ...
    static GetNote() {
        return axios
            .get('http://18.116.242.41:9000/Notes',{
                headers: {Authorization: localStorage.getItem("token") },
            }).catch((e) => {
                console.log(e)
            })
    ...
    static UpdateNote(id, title, content) {  
        return axios
            .patch(`http://18.116.242.41:9000/Notes/${id}`,{
                title,
                content
            },{
                headers: {Authorization: localStorage.getItem("token") },
    ...
    static DeleteNote(id) { 
        return axios
            .delete(`http://18.116.242.41:9000/Notes/${id}`, 
            {
                headers: {Authorization: localStorage.getItem("token") },
            }).catch((e) => {
                console.log(e)
    ...
    static CreateEvents(title, description, url, start, end, onSuccess) { 
        return axios
            .post('http://18.116.242.41:9000/Calendar', {
                title,
                description,
                url,
                start,
    ...
    static GetEvents() {
        return axios
            .get('http://18.116.242.41:9000/Calendar',{
                headers: {Authorization: localStorage.getItem("token") },
            }).catch((e) => {
                console.log(e)
            })
    ...
    static UpdateEvents(id, title, description, Url, start, end, startTime, endTime, daysOfWeek, display) {  
        return axios
            .patch(`http://18.116.242.41:9000/Calendar/${id}`,{
                title, 
                description, 
                Url, 
                start, 
    ...
    static DeleteEvents(id) {
        return axios
            .delete(`http://18.116.242.41:9000/Calendar/${id}`,{
                headers: {Authorization: localStorage.getItem("token") },
            }).catch((e) => {
                console.log(e)
            })
