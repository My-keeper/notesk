...
const CalenderContianer = (props) => {
  ...
  const StartingDataIcion = (
    <FadeIn>
      <div onClick={() => ChangeisStartedData(!isStartedData)}>
        {isStartedData ? (
  ...
  const EndingDataIcion = (
    <FadeIn>
      <div onClick={() => ChangeisEndedData(!isEndedData)}>
        {isEndedData ? (
          <VisibilityOffIcon style={VisibilityStle} />
        ) : (
          <VisibilityIcon style={VisibilityStle} />
  ...
  const InconArea = (
    ...
    >
      <FadeIn>
        <CalendarOutlined style={IconColor} />{" "}
      </FadeIn>
      {StartingDataIcion}
      {EndingDataIcion}
    </div>
  ...
  const InfoArea = (
    <EachateContainer JustifyContentCalue={"space-between"}>
      {isStartedData || isEndedData ? (
        Layout.map((value, index) => (
          <FadeIn key={index + createEventId()}>
            <span
              style={{
                padding: "1px",
                paddingTop: "1px",
  ...
  const StartingData = (
    <EachateContainer
      JustifyContentCalue={isStartedData ? "space-between" : ""}
    >
      <FadeIn>
        <span style={{ padding: "4px", fontSize: "1.2em", marginRight: "1px" }}>
          From
        </span>
      </FadeIn>
      {!isStartedData ? (
  ...
  const EndingingData = (
    <EachateContainer JustifyContentCalue={isEndedData ? "space-between" : ""}>
      <FadeIn>
        <span
          style={{ padding: "4px", fontSize: "1.2em", marginRight: "25px" }}
        >
          To
        </span>
      </FadeIn>
...
const TimerContainer = (props) => {
  ...
  const IcionArea = (
    ...
    >
      <FadeIn>
        <ClockCircleOutlined style={IconColor} />{" "}
      </FadeIn>
  ...
  const StartingTime = (
    <EachateContainer JustifyContentCalue={"space-between"}>
      <FadeIn>
        <span style={{ padding: "4px", fontSize: "1.2em", marginRight: "5px" }}>
          Start
        </span>
      </FadeIn>
      <TimeSelected
  ...
  props.FromHourSelected(
    StartDayValue === "PM"
      ? String(parseInt(StartHoursValue, 10) + 12)
      : StartHoursValue
  ); //this where we convert the returned hour from 12 hour system to 24 hour system
